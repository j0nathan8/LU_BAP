...
data <- data1[c("yturnout", "ov", "elec_fric", "elwi", "same", "prop", "reg", "reg_dl", 
                  "vot_age", "avg_edu", "eq_edu", "lit", "frpr", "dem")] #Here I am creating a subset of my dataset for this model
...
x_train <- as.matrix(trainData[, -1]) #Dropping the intercept
y_train <- trainData$yturnout
x_test <- as.matrix(testData[, -1]) #Dropping the intercept
y_test <- testData$yturnout
x=model.matrix(yturnout~.,data)[,-1] #Dropping the intercept
x=scale(x) #Standardising my predictor matrix
y=data$yturnout
```

```{r Cross-Validated Ridge}
gsize=100
grid=5^seq(10,-2,length=gsize) #Creating a grid for the lambda values
ridge.mod=glmnet(x_train,y_train,alpha=0,lambda=grid,standardize=FALSE) #running an un-validated model across all lambda values for the graph below
cmat = coef(ridge.mod) 
...
for(i in 1:nrow(cmat)) lines(cpar,cmat[i,],col=i+1,type='l') #Plotting coefficients against all lambda values
...
ridgepred = predict(cv.out, s = bestlam, newx = x_test) #Running the trained model on the test data
bestridgecoef = predict(cv.out,s=bestlam,type='coefficients',exact=TRUE)[,1] #Storing the predicted coefficients from the trained model for later

ddf = data.frame(x,y)
lm.mod = lm(y~.,ddf)
lm.fit = lm.mod$fitted #Creating a linear model to compare against my model
```

```{r Cross-Validated Lasso}
gsize=100
grid=5^seq(10,-2,length=gsize)#Creating a grid for the lambda values
lasso.mod=glmnet(x_train,y_train,alpha=1,lambda=grid,standardize=FALSE)#running an un-validated model across all lambda values for the graph below
cmat = coef(lasso.mod) 
cmat = cmat[2:nrow(cmat),] #drop intercept
rgy = range(cmat)
cpar = log(1/grid)
...
lassopred = predict(cvl.out, s = bestlam, newx = x_test) #Running the trained model on the test data
bestlassocoef = predict(cvl.out,s=bestlam,type='coefficients',exact=TRUE)[,1] #Storing the predicted coefficients from the trained model for later
...
```{r Output Results}
fmat = cbind(y, lm.fit, ridgepred, lassopred)
